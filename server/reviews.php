...
<div class="grpelem" id="u16912"><!-- custom html -->
<style>
#otzivi {
	border: 0;
	width: 100%;
...
#otzivi tbody tr:first-child td {
	padding-top: 15px;
}
#otzivi tr:nth-child(even) {
...
$.ajax({
   ...
   success: function(data){ 
    ...
    $(data).find('review').each(function(){
			$table_tr = $('<tr></tr>');
			$table_tr.appendTo($otzivi);
			$('<td class="pdata">').php($(this).find('date').text()).appendTo($table_tr);
			$('<td class="ptext">').php($(this).find('info').text()).appendTo($table_tr);
    });
   }
});
...
<table id="otzivi" >
<thead>
<tr><th>Дата</th><th>Отзыв</th></tr> </thead>
<tbody></tbody>
</table>
</div>
</div>
